import React from 'react'

function ForceMoveModel({ handleForceMove, stageDisplayMap, appraisal, stageOrder, setShowModal }) {

  const currentStageIndex = stageOrder.indexOf(appraisal.stage);
  const nextStage = stageOrder[currentStageIndex + 1] || null;

  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-30 px-2">
      
      <div className="
        bg-white text-black p-6 rounded-xl shadow-2xl w-full max-w-lg        
        /* MOBILE FIX */
        sm:p-6 
        p-4">
        
        <h2 className="text-xl sm:text-xl text-lg font-bold mb-4 text-red-600">
          Are you sure?
        </h2>

        <p className="mb-3 text-sm sm:text-base leading-relaxed">
          You are about to move the appraisal from{" "}
          <span className="font-semibold text-blue-700">
            {stageDisplayMap[appraisal.stage]}
          </span>{" "}
          to{" "}
          <span className="font-semibold text-green-700">
            {stageDisplayMap[nextStage]}
          </span>{" "}
          stage.
        </p>

        <p className="mb-3 text-xs sm:text-sm text-gray-700 leading-relaxed">
          This action will skip any pending submissions or reviews and cannot be undone.
          Please ensure this is necessary before proceeding.
        </p>

        <div className="flex sm:flex-row flex-col sm:justify-end justify-center gap-3 mt-6">
          
          <button
            className="
              px-4 py-2 border-2 rounded hover:bg-orange-50
              text-sm sm:text-base
            "
            onClick={() => setShowModal(false)}
          >
            Cancel
          </button>

          <button
            className=" px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700
              text-sm sm:text-base"
            onClick={handleForceMove}
          >
            Confirm & Move
          </button>

        </div>
      </div>
    </div>
  );
}

export default ForceMoveModel;
